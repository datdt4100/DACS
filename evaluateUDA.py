...
def get_iou(data_list, class_num, dataset, save_path=None):
    ...
    pbar = tqdm(data_list, total=len(data_list))
    for data in pbar:
        m = ConfM.generateM(data)
        ConfM.addM(m)
...
def evaluate(model, dataset, ignore_label=250, save_output_images=False, save_dir=None, input_size=(512,1024)):
    ...
    for index, batch in pbar:
        ...
        with torch.no_grad():
            ...
            if dataset == 'cityscapes':
                gt = np.asarray(label[0].numpy(), dtype=np.int_)
            elif dataset == 'gta':
                gt = np.asarray(label[0].numpy(), dtype=np.int_)

            output = output.transpose(1,2,0)
            output = np.asarray(np.argmax(output, axis=2), dtype=np.int_)
